...
#motor.throttle(throttle_left, throttle_right)
#motor.release()
#motor.open_gripper()
#motor.close_gripper()
    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/stop':
            ...
            if query_components is not None:
                speed1 = query_components["speed1"]
                speed2 = query_components["speed2"]
            motor.throttle(float(speed1), float(speed2))
...
#    my_server.serve_forever()
    print('PEM pass phrase is: pass')
    address = ('', 443)
    my_server = StreamingServer(address, StreamingHandler)
    my_server.socket = ssl.wrap_socket (my_server.socket,
        keyfile="key.pem",
